...
globalIter = 2							# 1000 NMF runs for each guess of a sources
nmfIter = 5									# 80,000 max number of iterations for each source.
locIter = 5										# 1000 minimizations are performed to find the location
...
aic_min, nopt = ShiftNMFk.AIC_final(Norm, Sil, numT, nd)		# nopt returns the correct number of sources in the experiment.
