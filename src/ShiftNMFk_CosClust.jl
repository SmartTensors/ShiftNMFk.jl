...
function ShiftNMFk_CosCluster(X, maxSource)
	...
	for z = Trials
			...
			@show size(f)
			for i=1:size(W,1)
				@show size(Hf[usecomp,:])
				@show size(T[i:i,usecomp])
				@show size(f)
				@show size(exp.(T[i:i,usecomp]'.*f))
				Hft=Hf[usecomp,:].*exp.(T[i:i,usecomp]'.*f) # have no idea what is this doing
				Hft=[Hft conj(Hft[:,end-1:-1:2])]
				Ht=real(ifft(Hft,2))
				@show size(Ht)
				@show size(W[i:i,usecomp])
